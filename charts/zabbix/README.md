...
| zabbixProxy.minReadySeconds | int | `0` | Minimum seconds a newly created Zabbix Proxy pod must stay Ready before the StatefulSet controller treats it as available and proceeds to the next replica during a rolling update. A Zabbix Proxy reports Ready as soon as it starts, but it still has to download its configuration from the Zabbix Server before it polls anything, so with the default of 0 all replicas can be restarted back to back and leave a window where none of them is collecting data. Reference: [https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/#minimum-ready-seconds](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/#minimum-ready-seconds) |
...
| zabbixProxy.updateStrategy | object | `{}` | Update strategy of the Zabbix Proxy StatefulSet. Left empty, the Kubernetes default (``RollingUpdate`` with ``partition: 0``) applies. Set ``rollingUpdate.partition`` to stage a rollout across replicas manually, or ``OnDelete`` to restart proxies by hand. Reference: [https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/#update-strategies](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/#update-strategies) |
